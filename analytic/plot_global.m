...
filename2 = strcat(datpath , 'stp.txt')
filename3 = strcat(datpath , 'stt.txt')
...
s = textscan(f2, 'eps:%f action:[%f  %f %f %f %f %f] reward:%f info:None state:[  %f   %f   %f   %f   %f]');
fclose(f2);
...
saveas(gcf, strcat(figpath, 'metrics.jpg'), 'jpg')
...
saveas(gcf, strcat(figpath, 'action.jpg'), 'jpg')

figure(3)
for k = 1:10
    plot(stt(:,1), stt(:, [2]) + 3 * k,'.')
    hold on
end
