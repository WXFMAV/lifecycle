close(figure(1))
fid = fopen('met.txt','r');
m = textscan(fid, 'eps:%f item_count:%f item_new:%f item_dead:%f id:%f t:%f q:%f ctr:%f ctr_avg:%f Q_all:%f startup_count:%f maturity_count:%f decline_count:%f startup_score:%f maturity_score:%f decline_score:%f reward:%f info:None');
...
subplot(4,2,5)
p1 = plot(m{1}, m{11}./(m{11}+m{12}+m{13}), 'r');
n = length(m{1});
hold on
plot(m{1}, m{14}, 'b');
plot(m{1}, m{15}, 'g');
