...
figurepath = './fig/'
datpath = './dat/'
filename = [datpath + 'param_high_dim1d/critic_loss.txt', datpath + 'param_high_dim1d/critic_loss.txt']
figfile = figurepath + 'critic_loss'
...
for k in range(len(data)):
    for j in range(data[k].shape[0]):
        #data[k][j, 1] = data[k][j, 1] if data[k][j, 1] < 0.1 else 0.9 * data[k][j - 1, 1] + 0.1 * data[k][j, 1]
        data[k][j, 1] = data[k][j, 1] if data[k][j, 1] - data[k][j-1,1] < 0.05 else data[k][j - 1, 1]
...
for k in range(0, len(sep) - 1):
    ...
    for i in range(0, len(data)):
        avg = tau * res[k - 1, 1 + i * 4] + (1-tau) * np.mean(data[i][sep[k]:sep[k+1],1]) if k > 0 else np.mean(data[i][sep[k]:sep[k+1],1])
#        res[k, 1 + i * 4] = avg
...
llabel = ['critic_loss1', 'critic_loss2','critic_loss3', 'critic_loss4']
...
plt.ylabel("critic_loss", fontsize=13, fontweight=glb_fontweight)
