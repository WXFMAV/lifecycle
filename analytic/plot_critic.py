...
figurepath = './fig/'
datpath = './dat/'
filename = datpath + 'critic_loss.txt'
...
y_min = 0.0 #float(np.min(data[:, 1], axis = 0))
y_max = 0.4 #float(np.max(data[:, 1], axis = 0)) + 0.5
...
plt.plot(res[:, 0], res[:, 1], color='0.2', linewidth=1.0, label='critic loss')
...
plt.ylabel("loss", fontsize=13, fontweight=glb_fontweight)
