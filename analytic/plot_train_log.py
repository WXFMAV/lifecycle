...
x_min_log = np.log2(x_min+0.0001)
x_max_log = np.log2(x_max) + 1.0
...
sep = []
for k in range(0, (60) * sample_step, 100):
    sep.append(k)
for k in range(60, sample_times - 1):
...
xspace = np.arange(np.min(res[:,0]), np.max(res[:, 0])+0.1, 2.0)
yspace = np.arange(y_min, y_max, float((y_max - y_min)/5.0))

group_labels = []
for v in xspace:
    if v >= 3.0:
        group_labels.append('%.d'%(round(np.power(2.0, v)/1000))+'k')
    else:
        group_labels.append('%.d' % (round(np.power(2.0, v))))
...
plt.xlim(x_min_log, x_max_log)  # 设置x轴的范围
...
plt.savefig(figure_path+figurename, format='pdf')  # 建议保存为svg格式，再用inkscape转为矢量图emf后插入word中
plt.show()