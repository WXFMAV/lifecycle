...
class ActorNetwork:
    ...
    def create_training_method(self):
        ...
        self.optimizer = tf.train.AdamOptimizer(float(self.param['LEARNING_RATE'])).apply_gradients(zip(self.parameters_gradients,self.net))

    def create_network(self,state_dim,action_dim):
        layer1_size = int(self.param['LAYER1_SIZE'])
        layer2_size = int(self.param['LAYER2_SIZE'])
    ...
    def create_target_network(self,state_dim,action_dim,net):
        state_input = tf.placeholder("float",[None,state_dim])
        ema = tf.train.ExponentialMovingAverage(decay=1-float(self.param['TAU']))
