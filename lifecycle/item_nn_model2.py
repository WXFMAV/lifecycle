...
def create_onechanel_network(input_dim, output_dim, layer_size = (24, 12)):
    W1 = tf.Variable(tf.random_uniform([input_dim, 24], -3e-3, 3e-3), name = 'W1')
    b1 = tf.Variable(tf.random_uniform([24], -3e-3, 3e-3), name = 'b1')
    W2 = tf.Variable(tf.random_uniform([24, 1], -3e-3, 3e-3), name = 'W2')
    b2 = tf.Variable(tf.random_uniform([1], -3e-3, 3e-3), name = 'b2')
    x = tf.placeholder('float', [None, input_dim])
    layer1 = tf.nn.relu(tf.matmul(x, W1) + b1)
    layer2 = tf.matmul(layer1, W2) + b2
    ypredict = layer2
    y = tf.placeholder('float', [None, 1])
    loss = tf.reduce_mean(tf.reduce_sum(tf.square(y - ypredict), reduction_indices = [1]))
    opt = tf.train.GradientDescentOptimizer(learning_rate = 0.1).minimize(loss)
    return x, [y], ypredict, [loss], [opt], [], [W1, b1, W2, b2]
def create_multitask_network(input_dim, output_dim, layer_size = (48, 24, 12, 6)):
    #create network
    layer1_size = layer_size[0]
    layer2_size = layer_size[1]
#    lbd = -1.0/math.sqrt(layer1_size)
#    ubd = 1.0/math.sqrt(layer1_size)
    ...
    for k in range(output_dim):
        ...
        W3 = tf.Variable(tf.random_uniform([layer2_size, layer3_size], lbd3, ubd3), name='W3' + str(k))
        b3 = tf.Variable(tf.random_uniform([layer3_size], lbd3, ubd3), name='b3'+str(k))
        W4 = tf.Variable(tf.random_uniform([layer2_size, layer4_size], lbd4, ubd4), name='W4' + str(k))
        b4 = tf.Variable(tf.random_uniform([layer4_size], lbd4, ubd4), name='b4' + str(k))
        W5 = tf.Variable(tf.random_uniform([layer4_size, layer5_size], lbd4, ubd4), name='W5' + str(k))
        b5 = tf.Variable(tf.random_uniform([layer5_size], lbd5, ubd5), name='b5' + str(k))
 
        layer3 = tf.nn.tanh(tf.matmul(layer2, W3) + b3)
        layer4 = tf.nn.relu(tf.matmul(layer2, W4) + b4)
        layer5 = tf.matmul(layer4, W5) + b5
        y.append(tf.placeholder("float", [None, 1]))
        yk.append(layer5)
        #loss.append(tf.reduce_sum(tf.square(y[k] - yk[k]), reduction_indices=[1]))
        ...
        opt.append(tf.train.GradientDescentOptimizer(learning_rate = 0.6).minimize(loss[k]))
...
class ItemModel(object):
    def __init__(self, param):
        ...
        self.net = net
        self.sess.run(tf.initialize_variables(self.net))
#        self.set_min_max(1)
#        self.restore_para()
    ...
    def save_para(self):
        tf.train.Saver(self.net).save(self.sess, self.param['filepath_im_model2'])
        

    def restore_para(self):
        tf.train.Saver(self.net).restore(self.sess, self.param['filepath_im_model2'])
...
def generate_samples(param):
    ...
    f_w = np.random.rand(int(param['im_output_dim']), int(param['im_feature_dim'])) * 1.0
...
#    opt = tf.train.AdamOptimizer().minimize(loss)
    ...
    x, y, yall, loss, opt, yshared, net = create_multitask_network(int(param['im_feature_dim']), int(param['im_output_dim']))
    #x, y, yall, loss, opt, yshared, net = create_onechanel_network(int(param['im_feature_dim']), int(param['im_output_dim']))
    epoch = 2000

    sess = tf.Session()
    sess.run(tf.initialize_variables(net))
...
#                        y[k]:y_batch[:, k].reshape(N, 1)
    ...
    print 'shape of yans:', np.shape(yans), np.shape(y_batch) 
#    print 'yans:', yans
    res = np.concatenate((yans, y_batch, x_batch), axis = 1) 
    f = open('dat/model_test.txt', 'w')
